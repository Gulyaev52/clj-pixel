(ns pixel-art.timeline.views
  (:require
   ...
   [pixel-art.views.ui-kit :refer [icon-button typography]]
    ...
    [:div {:class (css {:display "flex"
     ...
     [:div {:ref ref
            ...
            :class (css {:display "flex"
                         ...
                         :border-style "solid"
                         :cursor "pointer"})
            :style {:height cel-height
                    :border-color (get-border-color layer)
                    :border-width (if (:current layer) "2px" "1px")}}
    ...
    [:div {:class (css {:position "sticky"
     ...
     [:div {:on-click (fn [] (re-frame/dispatch [::events/select-frame (:idx frame)]))
            ...
            :class (css {:display "flex"
                         ...
                         :text-align "center"
                         :cursor "pointer"})
            :style {:border-style (if (:onion-skin frame) "dashed" "solid")
                    :border-color (get-border-color frame)
                    :border-width (if (:current frame) "2px" "1px")}}
  ...
  (let [[_ ref] (react-dnd/useDrag (fn []
    ...
    [:div {:class (css {:position "relative"})}
     ...
     [:div {:on-click (fn [e]
            ...
            :style {:border-color (get-border-color cel)
                    :border-width (if (:selected cel) "2px" "1px")}}
      ...
      [:div {:data-testid (str "cel-group-" (-> cel :pos :frame-idx) "-" (-> cel :pos :layer-idx))
             :class (css {:position "absolute" :top "0"})}
    ...
    [:div {:ref (:container-ref vertical-resizer-refs)
     ...
     [:div {:ref timeline-container-ref
            :class (css {:display "grid"
                         ...
                         :margin-top "4px"
                         :overflow "auto"})
            :style {:grid-auto-rows cel-height
      ...
      [:div {:class (css {:display "flex"
                          ...
                          :top "0"
                          :background-color "var(--pixel-color-bg-container)"})}
       [icon-button {:src (if some-layer-visible :visibility :visibility-off)
                     :title "toggle all layers visibility"
                     :data-testid "btn-toggle-all-layers-visibility"
                     :size :sm
                     :on-click (fn [] (re-frame/dispatch [::events/toggle-all-layers-visibility]))}]
       [icon-button {:src (if some-layer-automatic-linking :link :link-off)
                     :title "toggle all layers automatic linking"
                     :data-testid "btn-toggle-all-layers-automatic-linking"
                     :size :sm
                     :on-click (fn [] (re-frame/dispatch [::events/toggle-all-layers-automatic-linking]))}]]
      [:div {:class (css {:position "sticky"
                          :z-index "1"
                          :top "0"
                          :background-color "var(--pixel-color-bg-container)"})}]
      ...
      (doall
       (for [layer layers]
         ...
         [:<>
          [:div {:class (css {:display "flex"
                              :align-items "center"})}
           [icon-button {:src (if (:visible? layer) :visibility :visibility-off)
                         :title "toggle layer's visibility"
                         :data-testid (str "btn-toggle-layer-visibility-" (:idx layer))
                         :size :sm
                         :on-click (fn [] (re-frame/dispatch [::events/toggle-layer-visibility (:idx layer)]))}]
           [icon-button {:src (if (:automatic-linking? layer) :link :link-off)
                         :title "toggle layer's automatic linking"
                         :data-testid (str "btn-toggle-layer-automatic-linking-" (:idx layer))
                         :size :sm
                         :on-click (fn [] (re-frame/dispatch [::events/toggle-layer-automatic-linking (:idx layer)]))}]]
          [layer-view layer]
