 ...
 (fn [db]
   (let [scale (:scale db)]
     (when (:pixels-grid-enabled db)
       (str "data:image/svg+xml,%3Csvg width='" scale "' height='" scale "' opacity='" (min (/ scale project-settings/max-zoom-scale) 1) "' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='" scale "' height='" scale "' vector-effect='non-scaling-stroke' fill='transparent' stroke='blue' /%3E%3C/svg%3E")))))
