...
(defn draw [db event]
  (let [{:keys [initial-mouse-down-pos]} db
    ...
    (doseq [[x y] (resize-pixel (aget line-pixels 0) pixel-size)]
      (preview/set-color! preview x y current-color))
    (doseq [[x y] (resize-pixel (aget line-pixels (- (count line-pixels) 1)) pixel-size)]
      (preview/set-color! preview x y current-color))
    ;; for each step along the line, draw an x centered on that pixel of size penSize 
    (doseq [[x y] line-pixels]
      (dotimes [i pixel-size]
        (let [new-x (+ (- x (.floor js/Math (/ pixel-size 2))) i)
              new-y (+ (- y (.floor js/Math (/ pixel-size 2))) i)]
          (preview/set-color! preview new-x new-y current-color))
        (let [new-x (+ (- x (.floor js/Math (/ pixel-size 2))) i)
              new-y (- (- (+ y (.ceil js/Math (/ pixel-size 2))) i) 1)]
          (preview/set-color! preview new-x new-y current-color))
        ;; draw an additional x directly next to the first to prevent unwanted dithering
        (when (not= i 0)
          (let [new-x (+ (- x (.floor js/Math (/ pixel-size 2))) i)
                new-y (- (+ (- y (.floor js/Math (/ pixel-size 2))) i) 1)]
            (preview/set-color! preview new-x new-y current-color))
          (let [new-x (+ (- x (.floor js/Math (/ pixel-size 2))) i)
                new-y (- (+ y (.ceil js/Math (/ pixel-size 2))) i)]
            (preview/set-color! preview new-x new-y current-color)))))
    preview))
