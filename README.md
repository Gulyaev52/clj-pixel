...
* [`src/pixel_art/`](src/pixel_art/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/pixel_art/core.cljs): contains the SPA entry point, `init`
* [`test/pixel_art/`](test/pixel_art/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `pixel-art` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pixel_art/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns pixel-art.example
  (:require [pixel-art.config :as config])
